...
function SettingPrompt(props) {
  const { classKey, data, stateArray, settingType, requestBody, settingIcon } =
    props;
  const [selectedValue, setSelectedValue] = useState(
    data.rangeState === 0 ? 1 : data.rangeState
  );
  const [hideState, setHideState] = useState(
    selectedValue === 1 ? false : true
  );
  ...
  return (
    ...
    >
      <div style={{ width: "400px" }}>
        ...
        <div style={{ fontSize: "small", color: `var(--aim-text-sub)` }}>
          {data.description}
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          top: "5px",
          right: "0",
          backgroundColor: hideState
            ? `var(--aim-emphasis-red)`
            : `var(--aim-fill-green)`,
        }}
      >
